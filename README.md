...
- Snowflake intelligence活用したデータ接点の増加
  ...
  - 取り組み
    ...
    - メタデータ調整による精度検証
    - セマンティックビューの整備による既存ダッシュボードと数値合致を目指す取り組み
  - 成果
    - 退職に伴いフィジビリ検証に留まってしまったため定量で出せなかった
    - 得た学びは整備したとて組織のデータ関心を高めないと改善サイクルを回すことはできない
      - シンプルに忙しい
      - データをどのように確認していけば手法を知らない
    - データ・セマンティックビューの整備と同時にどのように業務内ににビルトインしていくかという観点が重要
- Streamlit in Snowflake (SiS)の活用フィジビリ検証
  - 背景：SiSの強みとしてデータを持ち出さず簡便なアプリが構築できることがある。これはコンプライアンス対策として大変有効である。また現在TableauでのBI環境を整えているが、ライセンス毎に課金がされるため、データの民主化に伴いコストも増加する。これを防ぐため活用が有効か検証した。
  - 成果
    - Tableauからの移管により年間500万円くらいの削減効果が見込めそう（退職により移管まで実行できず）
    - Container Runtimeへの変更により50%のコスト削減を達成
    - compute poolの夜間停止TASKSの導入により\$390のコスト削減
